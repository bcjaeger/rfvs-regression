...
1. Variable selection method `jiang` has consistently high prediction accuracy but scales poorly to large datasets in terms of computational efficiency. E.g., the max time that this method required was `r aorsf:::beautifyTime(round(max(data_model$time[data_model$rfvs=='jiang'])))`

1. Variable selection method `aorsf` has very high prediction accuracy when the downstream model is an oblique random forest, and does not become computationally intractable with larger data. E.g., the max time this method required was `r aorsf:::beautifyTime(round(max(data_model$time[data_model$rfvs=='aorsf'])))`
