...
vis_datasets_smry <- function(df){
 ...
 p1 <- ggplot(df,(aes(x=cv)))+geom_histogram(binwidth = 1, color="white", fill="black") +
  ggtitle(expression(Coefficient~of~Variation~bgroup("(",frac(sigma,abs(mu)), ")")~Across~Outcomes))+
  ylab("Frequency") + xlab("Coefficient of Variation")+theme_bw() + geom_hline(yintercept=0)+
 ...
 p2 <- ggplot(df,(aes(x=number.of.features)))+geom_histogram(binwidth = 25,color="white",  fill="black") +
  ggtitle("Number of Predictors")+
  ylab("Frequency") + xlab("Predictors") + theme_bw() + geom_hline(yintercept=0)

 #Create plot for number of instances
 p3 <- ggplot(df,(aes(x=number.of.instances)))+geom_histogram(binwidth = 200,color="white",  fill="black") +
  ggtitle("Number of Observations")+
  ylab("Frequency") + xlab("Observations") + theme_bw() + geom_hline(yintercept=0) +
